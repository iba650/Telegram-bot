...
class VideoKickBot:
        ...
        try:
            ...
            async def kick_user():
                try:
                    ...
                    # Send notification
                    kick_msg = (
                        f"?? {user_name} was removed for not posting a video within "
    ...
    async def handle_video_message(self, update: Update, context):
        ...
        try:
            ...
            with self.pending_users_lock:
                ...
                if user_key in self.pending_users:
                    # Cancel the timer
                    timer_info = self.pending_users[user_key]timer_info['timer'].cancel()
    ...
    async def set_timer_command(self, update: Update, context):"""Handle /settimer command to change timeout duration."""
        try:
            ...
            try:
                ...
                if new_timeout < 10 or new_timeout > 600:
                    await update.message.reply_text("?? Timer must be between 10 seconds and 10 minutes (600 seconds).")
                    return
    ...
    async def status_command(self, update: Update, context):
        ...
        try:
            ...
            status_msg = (
                ...
                f"?? Timer: {self.timeout_seconds} seconds\n"
        ...
        except Exception as e:logger.error(f"Error in status_command: {e}")
    ...
    async def pause_command(self, update: Update, context):
        ...
        try:
            ...
            chat_member = await context.bot.get_chat_member(chat_id, user_id)
            if chat_member.status not in ['administrator', 'creator']:
                await update.message.reply_text("?? Only group admins can pause the bot.")
                return
            self.bot_paused = True
            await update.message.reply_text("?? Bot paused! New members won't be kicked until you use /resume")
    ...
    async def resume_command(self, update: Update, context):
        """Handle /resume command to resume the bot."""        try:
            # Check if user is admin
            chat_id = update.effective_chat.id
            user_id = update.effective_user.id
            chat_member = await context.bot.get_chat_member(chat_id, user_id)
            if chat_member.status not in ['administrator', 'creator']:
                await update.message.reply_text("?? Only group admins can resume the bot.")
                return
            self.bot_paused = False
            await update.message.reply_text("?? Bot resumed! Video verification is now active.")
    ...
    async def stats_command(self, update: Update, context):
        ...
        try:
            ...
            stats_msg = (
                ...
                f"?? Current timer: {self.timeout_seconds} seconds\n"
                ...
                f"?? Suspicious kicked: {self.stats['suspicious_kicked']}\n"
    ...
    async def interaction_mode_command(self, update: Update, context):
        ...
        try:
            ...
            chat_member = await context.bot.get_chat_member(chat_id, user_id)
            if chat_member.status not in ['administrator', 'creator']:
                await update.message.reply_text("?? Only group admins can change bot settings.")return
    ...
    async def antispam_command(self, update: Update, context):
        ...
        try:
            ...
            if self.anti_spam_enabled:
                msg = "?? Anti-spam protection ON! Now blocking links, banned words, and suspicious users"
            else:
                msg = "?? Anti-spam protection OFF! Only video verification is active"
            await update.message.reply_text(msg)
...
?? Total Protection Actions: {total_actions}
? New Members: {self.stats['total_joins']}
? Verified: {self.stats['users_verified']}
? Kicked (no video): {self.stats['users_kicked']}
? Spam blocked: {self.stats['spam_blocked']}Links blocked: {self.stats['links_blocked']}
?? Suspicious users: {self.stats['suspicious_kicked']}
...
Status: {'Active' if not self.bot_paused else 'Paused'}"""
    ...
    async def schedule_command(self, update: Update, context):
        ...
        try:
            ...
            user_id = update.effective_user.id
            chat_member = await context.bot.get_chat_member(chat_id, user_id)
            if chat_member.status not in ['administrator', 'creator']:                await update.message.reply_text("?? Only group admins can change bot settings.")
                return
            if not context.args:
            ...
            if context.args[0].lower() == "toggle":
                ...
                try:
                    ...
                    await update.message.reply_text("?? Please provide valid hour numbers")
        ...
        try:
            ...
            if user_key not in self.pending_users and user_key not in self.verified_users:
                ...
                with self.pending_users_lock:
                    ...
                    self.pending_users[user_key] = {
                        'timer': timer,'join_time': datetime.now(),
                        'user_name': user_name
                    }
                    timer.start()
            ...
            if message.text:
                ...
                for banned_word in self.banned_words:
                    if banned_word in text_lower:
                        ...
                        return            # Check for suspicious usernames
    ...
    async def _handle_spam_violation(self, message, context, reason: str, stat_key: str):
        ...
        try:
            ...
            # Send notification
            warning_msg = f"?? {user_name} was removed for {reason}"
            await context.bot.send_message(
                chat_id=chat_id,
                text=warning_msg,
                disable_notification=True
